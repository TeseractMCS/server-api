...
export default function Optional(
    ...
    for (let i = 0; i < target.__optionalParameters[propertyKey].length; i++) {
        ...
        if (
            optionalIndex < paramTypes.length - 1 &&
            !target.__optionalParameters[propertyKey].includes(
                optionalIndex + 1,
            )
        ) {
            throw new Error(
                `Optional parameter at index ${optionalIndex} is followed by a required parameter at index ${
                    optionalIndex + 1
                } at '${String(propertyKey)}' in @CommandAlias: '${
