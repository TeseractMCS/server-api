...
export default function CommandHandler(player, commandName, ...args) {
    ...
    const command = commandClass.constructor["__commandData"] ?? commandClass.__commandData;
    let argsCopy = [...args];
    let data = command;
    ...
    while (current) {
        const subCommand = data.subCommands
            ? Object.values(data.subCommands).find((cmd) => cmd.aliases.includes(current) ||
                cmd.name == current)
            : undefined;
        if (subCommand) {
    ...
    if (potentialSubCommand && potentialSubCommand.callbacks) {
        for (const callback of potentialSubCommand.callbacks) {
            ...
            const paramTypes = Reflect.getMetadata("design:paramtypes", commandClass, callback)?.slice(1) ?? [];
            const optionalParams = commandClass.__optionalParameters?.[callback] || [];
            if (argsCopySub.length === paramTypes.length ||
                ...
                try {
                    const parsedArgs = argsCopySub
                        ?.slice(0, paramTypes.length)
                        .map((arg, index) => ArgumentParser(arg, paramTypes[index])) ?? [];
    ...
    const defaults = (commandClass.constructor.__commandData ?? commandClass.__commandData)
        .defaults || [];
    if (defaults.length > 0) {
        let bestMatch = null;
        let bestMatchCount = 0;
        for (const defaultMethod of defaults) {
            const paramTypes = Reflect.getMetadata("design:paramtypes", commandClass, defaultMethod)?.slice(1) ?? [];
            const optionalParams = commandClass.__optionalParameters?.[defaultMethod] || [];
            if (argsCopy.length === paramTypes.length ||
