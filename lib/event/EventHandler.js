...
export default function EventHandler(target, propertyKey, descriptor) {
    let constructor;
    ...
    if (!paramtypes || paramtypes.length === 0) {
        throw new Error(`Method "${propertyKey}" in class "${constructor.name}" must have at least one parameter for the @EventHandler decorator.`);
    }
    const unparsedName = paramtypes[0].name;
    const eventName = unparsedName[0].toLowerCase() +
        unparsedName
            .substring(1)
            .replace("Before", "")
            .replace("After", "")
    ...
    if (!world.afterEvents[eventName] && world.beforeEvents[eventName]) {
        throw new Error(`Event type '${unparsedName}' used in method "${propertyKey}" of class "${constructor.name}" is not registered in EventMap.`);
    }
